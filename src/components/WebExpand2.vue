...
q-list.q-mt-md(separator)
	q-expansion-item(v-for="(panel, index) in store.panelsWeb2" group="group" v-model="panel.expanded" switch-toggle-side :key="panel.id" :class="{ er: panel.neg }" :label="panel.title")
