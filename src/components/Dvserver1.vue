<template>
	<div>
		...
		<div class="bar">
			...
			<q-btn class="plus" flat round dense icon="mdi-plus-circle" @click="add">
				<q-tooltip>Добавить конфигурцию</q-tooltip>
			</q-btn>
			<q-space></q-space>
			<div class="alltab">
				<q-tabs class="text-secondary" v-model="store.tabs" active-color="primary" inline-label>
					<q-tab v-for="item in store.config" :key="item.id" :name="item.name" :label="item.name"></q-tab>
				</q-tabs>
			</div>
		</div>
		<q-card>
